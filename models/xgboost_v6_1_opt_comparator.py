"""
XGBoost v6.1 — Predicción mortalidad UCI COVID-19 a 30 días
...
LOW_COVERAGE_LABS_V61 = {'pt', 'crp', 'ferritin', 'dimer', 'lymphocytes', 'neutrophils', 'troponin'}
...
def _is_low_coverage_numeric(col):
    ...
    return lab_name in LOW_COVERAGE_LABS_V61
...
def optimize_and_train(dataset_path, option_name, optimize_for='F1', beta=2.0, n_trials=100):
    ...
    """
    print(f"\n{'='*70}")
    print(f"  XGBOOST V6.1 — {option_name} ({optimize_for})")
    print(f"{'='*70}\n")

    output_dir = f"v6_1-models/{option_name}/{optimize_for}"
    ...
    report_lines.append(f"REPORTE V6.1 — {option_name} ({optimize_for})\n")
    report_lines.append(f"{'='*70}\n\n")
    ...
    report_lines.append(f"  {'XGBoost v6.1':<30}: {auroc:.4f}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="XGBoost v6.1 con Optuna — Modular (OptionB/C × F1/F2)",
    ...
    print("\n" + "="*70)
    print("XGBOOST V6.1 — Configuración:")
    ...
    for option in options:
        for metric in metrics:
            ...
            possible_paths = [
                os.path.join(project_root, "notebooks", "datasets", "v6_1",
                             f"dataset_v61_option{option}.parquet"),
                os.path.join(project_root, "datasets", "v6_1",
                             f"dataset_v61_option{option}.parquet"),
                f"../notebooks/datasets/v6_1/dataset_v61_option{option}.parquet",
                f"notebooks/datasets/v6_1/dataset_v61_option{option}.parquet",
            ]
