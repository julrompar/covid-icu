...
f2_scores_val = np.array([
    fbeta_score(y_val, y_probs_val > thr, beta=2.0, pos_label=1, zero_division=0)
    for thr in thresholds_search
])
optimal_idx        = np.argmax(f2_scores_val)
optimal_threshold  = float(thresholds_search[optimal_idx])

print("=" * 50)
print("OPTIMIZACIÓN POR F2 (sobre validation set)")
print("=" * 50)
print(f"Umbral óptimo: {optimal_threshold:.4f}")
print(f"F2 máximo (val): {f2_scores_val[optimal_idx]:.4f}")

with open(os.path.join(output_dir, "best_params.json"), "r") as fp:
    saved = json.load(fp)
saved["optimal_threshold_f2"] = optimal_threshold
saved["val_f2_at_threshold"]  = float(f2_scores_val[optimal_idx])
...
print(f"\nClasificación umbral F2-óptimo ({optimal_threshold:.4f}):")
...
plt.plot(thresholds_search, f2_scores_val, color="purple", lw=2)
plt.axvline(optimal_threshold, color="red", linestyle="--",
            label=f"Umbral óptimo = {optimal_threshold:.3f}")
plt.xlabel("Umbral de Decisión"); plt.ylabel("F2-Score clase 1")
plt.title("Optimización del Umbral por F2-Score (validation set)")
plt.legend(); plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "f2_vs_threshold.png"), dpi=300, bbox_inches="tight")
plt.close()
...
sns.heatmap(cm_opt, annot=True, fmt="d", cmap="Greens", ax=axes[1],
            xticklabels=["Pred Vivo", "Pred Muerte"], yticklabels=["Real Vivo", "Real Muerte"])
axes[1].set_title(f"Matriz de Confusión — Umbral F2-óptimo ({optimal_threshold:.4f})")
axes[1].set_ylabel("Valor Real"); axes[1].set_xlabel("Valor Predicho")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "confusion_matrices_f2.png"), dpi=300, bbox_inches="tight")
plt.close()
